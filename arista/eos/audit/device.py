...
eosDeviceKeylist = [{'name': {'data': 'hostname', 'hl': 'Hostname'}},
                    {'mgmtip': {'data': 'mgmtip', 'hl': 'Mgmt IP'}},
                    {'serial': {'data': 'serialNumber', 'hl': 'S/N'}},
                    {'version': {'data': 'version', 'hl': 'Version'}},
                    {'mfg': {'data': 'mfg', 'hl': 'Vendor'}},
                    {'model': {'data': 'modelName', 'hl': 'Model'}},
                    {'uptime': {'data': 'uptime', 'hl': 'Uptime'}}]

eosVlanKeylist = [{'name': {'data': 'name', 'hl': 'VLAN Name'}},
                  {'vlanid': {'data': 'vlanid', 'hl': 'VLAN ID'}},
                  {'status': {'data': 'status', 'hl': 'Status'}},
                  {'ip': {'data': 'varp', 'hl': 'IP Gateway'}},
                  {'subnet': {'data': 'subnet', 'hl': 'Subnet'}},
                  {'vrf': {'data': 'vrf', 'hl': 'VRF'}},]
...
class eosDevice:
    ...
    def __init__(self, json_data):
        self.keylist = eosDeviceKeylist
        # process uptime
        self.data = json_data
        self.name = json_data['show_hostname']['hostname']
        self.serial = json_data['show_version']['serialNumber']
        self.version = json_data['show_version']['version']
        self.model = json_data['show_version']['modelName']
        self.mfg = json_data['show_version']['mfgName']
        self.uptime = json_data['show_version']['uptime']
        self.eth = []
        self.portchannel = []
        self.lo = []
        self.vlanInt = []
    ...
    def getVlans(self):
        ...
        for a in keys:
            ...
            try:
                ...
                tmpSubnet = ipcalc.Network(int_bri['primaryIp']['address'], mask=int_bri['primaryIp']['maskLen'])
                data[a]['subnet'] = str(tmpSubnet.network()) + '/' + str(int_bri['primaryIp']['maskLen'])
            except:
                data[a]['ip'] = ''
                data[a]['mask'] = ''
                data[a]['subnet'] = ''
...
def BuildDevice(json_data):
    device = eosDevice(json_data)
